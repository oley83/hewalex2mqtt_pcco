...
class BaseDevice:
    ...
    def _optimized_read_registers(self, ser, register_list):
        """
        Optymalizowany odczyt rejestrów: grupuje rejestry w ciągłe bloki.
        """
        if not register_list:
            return

        # Sortowanie i usuwanie duplikatów
        registers = sorted(set(register_list))

        # Tworzenie bloków ciągłych rejestrów (różnica 2 między kolejnymi, bo rejestry są co 2 bajty)
        blocks = []
        current_block = [registers[0]]
        for i in range(1, len(registers)):
            if registers[i] == registers[i-1] + 2:
                current_block.append(registers[i])
            else:
                blocks.append(current_block)
                current_block = [registers[i]]
        blocks.append(current_block)

        # Dla każdego bloku odczytaj w paczkach o wielkości nieprzekraczającej REG_MAX_NUM
        for block in blocks:
            start_addr = block[0]
            block_length = len(block)

            # Podziel blok na części, każda o długości max REG_MAX_NUM
            for i in range(0, block_length, self.REG_MAX_NUM):
                chunk_start = start_addr + i * 2
                chunk_size = min(self.REG_MAX_NUM, block_length - i)
                logger.debug(f"Reading optimized block: start={chunk_start}, size={chunk_size}")
                self.readRegisters(ser, chunk_start, chunk_size)

    def readStatusRegisters(self, ser):
        """
        Odczyt rejestrów statusowych z optymalizacją.
        """
        # Zbierz adresy rejestrów statusowych (od REG_STATUS_START do REG_CONFIG_START-1)
        status_registers = [
            addr for addr in self.registers.keys()
            if self.REG_STATUS_START <= addr < self.REG_CONFIG_START
        ]
        logger.debug(f"Optimized status registers reading: {len(status_registers)} registers in total")
        self._optimized_read_registers(ser, status_registers)

    def readConfigRegisters(self, ser):
        """
        Odczyt rejestrów konfiguracyjnych z optymalizacją.
        """
        # Zbierz adresy rejestrów konfiguracyjnych (od REG_CONFIG_START do REG_MAX_ADR)
        config_registers = [
            addr for addr in self.registers.keys()
            if self.REG_CONFIG_START <= addr <= self.REG_MAX_ADR
        ]
        logger.debug(f"Optimized config registers reading: {len(config_registers)} registers in total")
        self._optimized_read_registers(ser, config_registers)
    ...
    def parseRegisterValue(self, reg, val):
        if val:                
            if reg['type'] == 'date':
                ...
                val = None
            if reg.get('options') and val not in reg['options']:
