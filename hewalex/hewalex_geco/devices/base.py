...
class BaseDevice:
    ...
    def readStatusRegisters(self, ser):
        ...
        end = self.REG_CONFIG_START
        total_bytes = end - start
        num_registers = total_bytes // 2
        
        # Jeśli liczba rejestrów jest większa niż maksymalna, podziel na partie
        if num_registers > self.REG_MAX_NUM:
            current = start
            while current < end:
                remaining_bytes = end - current
                current_num_registers = min(remaining_bytes // 2, self.REG_MAX_NUM)
                self.readRegisters(ser, current, current_num_registers)
                current += current_num_registers * 2
                
                # Dodaj przerwę między partiami (oprócz ostatniej)
                if current < end and BATCH_DELAY > 0:
                    time.sleep(BATCH_DELAY)
        else:
            # Odczyt w jednej partii
            return self.readRegisters(ser, start, num_registers)

    def readConfigRegisters(self, ser):
        start = self.REG_CONFIG_START
        while start < self.REG_MAX_ADR:
            remaining_bytes = self.REG_MAX_ADR + 2 - start
            num_registers = remaining_bytes // 2
            num = min(num_registers, self.REG_MAX_NUM)
            self.readRegisters(ser, start, num)
            start += num * 2
            
            # Dodaj przerwę między partiami (oprócz ostatniej)
            if start < self.REG_MAX_ADR and BATCH_DELAY > 0:
                time.sleep(BATCH_DELAY)
